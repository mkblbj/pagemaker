    ...
    URL.revokeObjectURL(url);
     ...
     switch (module.type) {
       case PageModuleType.TITLE:
        ...
        const titleClass = level === 1 ? 'text-2xl' : 
                          level === 2 ? 'text-xl' : 
        ...
        return (
          <div className="space-y-2">
            ...
            <div className={`font-bold text-gray-800 ${titleClass}`}>
          ...
          <div className="space-y-2">
            ...
            >
              {Array.from({ length: columns }).map((_, index) => (
                <div key={index} className="border border-gray-200 rounded p-4 text-center text-sm text-gray-500 bg-gray-50">
                  列 {index + 1}
                  <br />
            ...
            {(page.created_at || page.updated_at) && (
              <CardContent className="pt-0">
                <div className="flex gap-4 text-sm text-muted-foreground">
                  {page.created_at && (
                    <div>创建时间: {new Date(page.created_at).toLocaleString('zh-CN')}</div>
                  )}
                  {page.updated_at && (
                    <div>更新时间: {new Date(page.updated_at).toLocaleString('zh-CN')}</div>
                  )}
                </div>
              </CardContent>
            )}
          ...
          <CardContent className="p-4 text-center text-sm text-muted-foreground">
            <div className="flex items-center justify-center gap-2">
              ...
              <span>
                当前处于只读模式 | 
                {modules.length} 个模块 | 
                浏览器: {compatibilityData.browser.name} {compatibilityData.browser.version}
              </span>
            </div>
          </CardContent>
        </Card>
