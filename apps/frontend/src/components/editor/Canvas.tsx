...
import { Plus, FileText, GripVertical, Trash2, Copy, MoveUp, MoveDown } from 'lucide-react'
import { PageModuleType } from '@pagemaker/shared-types'

export function Canvas() {
  const { currentPage, selectedModuleId, setSelectedModule, deleteModule, reorderModules, addModule } = usePageStore()

  const { isDragging, setDragging, markUnsaved } = useEditorStore()
        ...
        <div className="mb-6">
          <div className="bg-white rounded-lg shadow-sm border p-4">
            ...
            <p className="text-sm text-muted-foreground">
              {modules.length === 0 ? '从左侧拖拽模块到此处开始构建页面' : `共 ${modules.length} 个模块`}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {modules.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-600 mb-2">开始创建您的页面</h3>
              <p className="text-gray-500 mb-4">从左侧模块库拖拽模块到此处，或点击下方按钮添加模块</p>
              <Button
                variant="outline"
                onClick={() => {
                  // 添加一个默认的文本模块
            ...
            modules.map((module, index) => (
              <div key={module.id} className="relative">
                {/* 拖拽放置区域 */}
                {dragOverIndex === index && <div className="h-2 bg-primary/20 rounded-full mb-2" />}

                {/* 模块容器 */}
                <Card
                  className={`relative bg-white transition-all duration-200 ${
                    selectedModuleId === module.id ? 'ring-2 ring-primary shadow-lg' : 'hover:shadow-md'
                  }`}
                ...
                <div
                  ...
                  onDragLeave={handleDragLeave}
                >
                  {dragOverIndex === index + 1 && <div className="h-2 bg-primary/20 rounded-full" />}
                </div>
              </div>
            ))
          )}
