...
interface PropertiesPanelProps {
  ...
  onPropertyChange?: (moduleId: string, propertyPath: string, value: unknown) => void
  ...
  onKeyValueItemChange?: (moduleId: string, itemIndex: number, keyOrValue: 'key' | 'value', value: string) => void
}
...
export default function PropertiesPanel({
  ...
  const validateProperty = (name: string, value: unknown): string | null => {
    if (name === 'content' && (value === null || value === undefined || String(value).trim() === '')) {
      return '内容不能为空'
    }
    if (name.toLowerCase().includes('url') && value && !/^(ftp|http|https):\/\/[^ "]+$/.test(String(value))) {
      return '请输入有效的URL'
    }
    if (
    ...
    if (
      ...
      if (value !== 'auto' && (isNaN(Number.parseFloat(String(value))) || Number.parseFloat(String(value)) < 0)) {
        return '必须为非负数'
      }
    }
    return null
  ...
  const handleNestedInputChange = (baseProperty: string, nestedKey: string, value: unknown) => {
    if (!selectedModule) return
  ...
  if (!selectedModule) {
    return (
      <div className="flex flex-col h-full items-center justify-center p-6 bg-white text-gray-500">
        ...
        <p className="text-xs text-center mt-1">请在画布中选择一个模块以编辑其属性。</p>
      </div>
    )
  }
      ...
      <div key={propKey} className="space-y-1.5">
        ...
        {type === 'number' && (
          <Input
            ...
            type="number"
            value={String(propValue ?? '')}
            onChange={e => handleInputChange(propKey, e.target.value === '' ? null : Number.parseFloat(e.target.value))}
            className={cn(error && 'border-red-500')}
            aria-describedby={error ? `${inputId}-error` : undefined}
          />
        )}
        ...
        {type === 'select' && options && (
          <Select value={String(propValue ?? '')} onValueChange={value => handleInputChange(propKey, value)}>
        ...
        {type === 'spacing' && (
          <SpacingInputGroup
            ...
            onChange={(key, value) => handleNestedInputChange(propKey, key, value)}
          />
        )}
        {error && (
      ...
      <div className="space-y-3">
        {items.map((item, index) => (
          <div key={index} className="p-3 border border-gray-200 rounded-md space-y-2 bg-gray-50">
            ...
            {renderPropertyField(`items[${index}].key`, item.key, '键 (Key)', 'text')}
            {renderPropertyField(`items[${index}].value`, item.value, '值 (Value)', 'textarea')}
          </div>
        ))}
        <Button variant="outline" size="sm" onClick={() => onAddKeyValueItem?.(selectedModule.id)} className="w-full">
          添加键值对
        </Button>
      </div>
    )
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{selectedModule.name || '模块属性'}</h2>
          </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          ...
          {selectedModule.type === 'title' && (
            ...
            >
              {renderPropertyField('content', selectedModule.properties.content, '标题文本', 'textarea')}
              {renderPropertyField('level', selectedModule.properties.level, '标题级别', 'select', [
                { value: 'h1', label: 'H1' },
                { value: 'h2', label: 'H2' },
                { value: 'h3', label: 'H3' },
                { value: 'h4', label: 'H4' },
              ...
              {renderPropertyField('alignment', selectedModule.properties.alignment, '对齐方式', 'select', [
                { value: 'left', label: '居左' },
                { value: 'center', label: '居中' },
                { value: 'right', label: '居右' }
              ])}
              {renderPropertyField('color', selectedModule.properties.color, '文字颜色', 'color')}
            </CollapsibleSection>
          )}
          ...
          {selectedModule.type === 'text' && (
            <CollapsibleSection
              ...
              onToggle={toggleSection}
            >
              {renderPropertyField('content', selectedModule.properties.content, '文本内容', 'textarea')}
              {renderPropertyField('fontSize', selectedModule.properties.fontSize, '字体大小 (px)', 'number')}
              {renderPropertyField('alignment', selectedModule.properties.alignment, '对齐方式', 'select', [
                { value: 'left', label: '居左' },
                { value: 'center', label: '居中' },
                { value: 'right', label: '居右' },
          ...
          {selectedModule.type === 'image' && (
            <CollapsibleSection
              ...
              onToggle={toggleSection}
            >
              {renderPropertyField('imageUrl', selectedModule.properties.imageUrl, '图片URL', 'text')}
              {renderPropertyField('altText', selectedModule.properties.altText, '替代文本', 'text')}
              {renderPropertyField('width', selectedModule.properties.width, '宽度 (%)', 'number')}
          ...
          {selectedModule.type === 'separator' && (
            <CollapsibleSection
              ...
              onToggle={toggleSection}
            >
              {renderPropertyField('lineStyle', selectedModule.properties.lineStyle, '线条样式', 'select', [
                { value: 'solid', label: '实线' },
                { value: 'dashed', label: '虚线' },
                { value: 'dotted', label: '点线' }
              ])}
              {renderPropertyField('color', selectedModule.properties.color, '线条颜色', 'color')}
              {renderPropertyField('thickness', selectedModule.properties.thickness, '线条粗细 (px)', 'number')}
            </CollapsibleSection>
          )}
          ...
          {selectedModule.type === 'keyValue' && (
            ...
            >
              {renderKeyValueItems()}
              {renderPropertyField('itemStyle', selectedModule.properties.itemStyle, '列表样式', 'select', [
                { value: 'default', label: '默认' },
                { value: 'bullet', label: '项目符号' },
                { value: 'numbered', label: '数字编号' }
              ])}
              {renderPropertyField('keyColor', selectedModule.properties.keyColor, '键 (Key) 颜色', 'color')}
              {renderPropertyField('valueColor', selectedModule.properties.valueColor, '值 (Value) 颜色', 'color')}
            </CollapsibleSection>
          )}

          {/* Common Spacing Section */}
          <CollapsibleSection title="间距" sectionKey="spacing" isOpen={openSections.spacing} onToggle={toggleSection}>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              {renderPropertyField('marginTop', selectedModule.properties.marginTop, '上外边距 (px)', 'number')}
              {renderPropertyField('marginBottom', selectedModule.properties.marginBottom, '下外边距 (px)', 'number')}
              {renderPropertyField('paddingTop', selectedModule.properties.paddingTop, '上内边距 (px)', 'number')}
              {renderPropertyField('paddingBottom', selectedModule.properties.paddingBottom, '下内边距 (px)', 'number')}
              {renderPropertyField('paddingLeft', selectedModule.properties.paddingLeft, '左内边距 (px)', 'number')}
              {renderPropertyField('paddingRight', selectedModule.properties.paddingRight, '右内边距 (px)', 'number')}
            </div>
          </CollapsibleSection>
          ...
          <CollapsibleSection
            ...
            onToggle={toggleSection}
          >
            {renderPropertyField('backgroundColor', selectedModule.properties.backgroundColor, '背景颜色', 'color')}
          </CollapsibleSection>
          ...
          {Object.values(errors).some(err => err) && (
            <Alert variant="destructive">
              ...
              <AlertDescription>表单中存在一些错误，请检查并修正。</AlertDescription>
            </Alert>
          )}
        </div>
      </ScrollArea>
...
const CollapsibleSection = ({ title, sectionKey, isOpen, onToggle, children }: CollapsibleSectionProps) => (
...
const ColorPickerInput = ({ id, value, onChange, error }: ColorPickerInputProps) => {
  ...
  const handleCustomColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^#([0-9A-Fa-f]{3}){1,2}$|^transparent$|^inherit$/.test(newColor) || newColor === '') {
      onChange(newColor)
    }
  }
...
interface SpacingInputGroupProps {
  ...
  onChange: (key: 'top' | 'bottom' | 'left' | 'right', value: number | null) => void
}
const SpacingInputGroup = ({ idPrefix, values, onChange }: SpacingInputGroupProps) => {
  const spacingLabels = { top: '上', bottom: '下', left: '左', right: '右' }
  return (
    <div className="grid grid-cols-2 gap-x-3 gap-y-2">
      {(['top', 'bottom', 'left', 'right'] as const).map(side => (
        <div key={side}>
          <Label htmlFor={`${idPrefix}-${side}`} className="text-xs text-gray-500">
          ...
          <Input
            ...
            onChange={e => onChange(side, e.target.value === '' ? null : Number.parseFloat(e.target.value))}
            className="mt-0.5"
            placeholder="0"
          />
        </div>
