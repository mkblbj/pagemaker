...
export interface PageModule {
  id: string
  type: 'title' | 'text' | 'image' | 'separator' | 'keyValue' | 'multiColumn' | string // Allow custom types
  name: string
  description: string
...
interface ModuleListProps {
  modules: PageModule[]
  onModuleDragStart?: (event: React.DragEvent<HTMLDivElement>, module: PageModule) => void
...
export default function ModuleList({ modules = mockModules, onModuleDragStart, onModuleDragEnd }: ModuleListProps) {
  ...
  const handleDragStart = (event: React.DragEvent<HTMLDivElement>, module: PageModule) => {
    ...
    <div className="flex flex-col h-full bg-white text-gray-800">
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6">
          {(Object.keys(categorizedModules) as Array<keyof typeof categorizedModules>).map(categoryKey => {
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            draggingModuleId === module.id && 'opacity-50 shadow-lg'
                          )}
          ...
          {filteredModules.length === 0 && searchTerm && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-600">未找到与 &ldquo;{searchTerm}&rdquo; 相关的模块</p>
              <p className="text-xs text-gray-400 mt-1">请尝试其他关键词或清空搜索框。</p>
            </div>
          )}
        </div>
      </ScrollArea>
