...
class AppsStructureTestCase(TestCase):
    ...
    def test_users_app_configuration(self):
        ...
        for file_name in required_files:
            ...
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在 users 应用中不存在"
            )
    ...
    def test_pages_app_configuration(self):
        ...
        for file_name in required_files:
            file_path = os.path.join(app_path, file_name)
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在 pages 应用中不存在"
            )
    ...
    def test_media_app_configuration(self):
        ...
        for file_name in required_files:
            file_path = os.path.join(app_path, file_name)
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在 media 应用中不存在"
            )
    ...
    def test_api_app_configuration(self):
        ...
        for file_name in required_files:
            file_path = os.path.join(app_path, file_name)
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在 api 应用中不存在"
            )
    ...
    def test_migrations_directories_exist(self):
        ...
        for app_name in local_apps:
            ...
            self.assertTrue(
                os.path.exists(migrations_path), f"迁移目录在 {app_name} 应用中不存在"
            )
...
class ProjectStructureTestCase(TestCase):
    ...
    def test_project_root_files_exist(self):
        ...
        for file_name in required_files:
            ...
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在项目根目录中不存在"
            )
    ...
    def test_pagemaker_package_structure(self):
        ...
        required_files = ["__init__.py", "settings.py", "urls.py", "wsgi.py", "asgi.py"]

        for file_name in required_files:
            file_path = os.path.join(pagemaker_dir, file_name)
            self.assertTrue(
                os.path.exists(file_path), f"文件 {file_name} 在 pagemaker 包中不存在"
            )
    ...
    def test_tests_directory_structure(self):
        ...
        self.assertTrue(
            os.path.exists(init_file), "__init__.py 文件在 tests 目录中不存在"
        )
