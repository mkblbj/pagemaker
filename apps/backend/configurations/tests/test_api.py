...
class ShopConfigurationAPITests(TestCase):
    ...
    def setUp(self):
        ...
        # 创建普通用户的UserProfile
        UserProfile.objects.create(
            user=self.regular_user, role="editor", full_name="Regular User"
        )
        ...
        self.detail_url = reverse(
            "configurations:shop-configuration-detail", kwargs={"id": self.config.id}
        )
    ...
    def test_create_shop_configuration_as_admin(self):
        ...
        # 验证数据库中是否创建成功
        self.assertTrue(
            ShopConfiguration.objects.filter(target_area="new_test_area").exists()
        )
