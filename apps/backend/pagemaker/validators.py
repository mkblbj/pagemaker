...
from .schemas import (
    UserModel, PageTemplateModel, ShopConfigurationModel,
    ApiResponse, ErrorResponse, SuccessResponse, PaginatedResponse
)
...
USER_SCHEMA = {
    ...
    "required": ["id", "username", "email", "fullName", "role", "isActive", "createdAt", "updatedAt"],
...
PAGE_MODULE_SCHEMA = {
    ...
    "properties": {
        "id": {"type": "string"},
        "type": {"type": "string", "enum": ["title", "text", "image", "separator", "keyValue", "multiColumn"]}
    },
    "required": ["id", "type"],
...
PAGE_TEMPLATE_SCHEMA = {
    ...
    "required": ["id", "name", "content", "targetArea", "ownerId", "createdAt", "updatedAt"],
    "additionalProperties": False
}
...
class PydanticValidator:
    ...
    @staticmethod
    def validate_page_template_model(data: Dict[str, Any]) -> Optional[PageTemplateModel]:
        """验证并转换页面模板模型"""
        try:
    ...
    @staticmethod
    def validate_shop_configuration_model(data: Dict[str, Any]) -> Optional[ShopConfigurationModel]:
        """验证并转换店铺配置模型"""
        try:
...
def create_api_response(success: bool, data: Any = None, message: str = None, errors: Dict[str, List[str]] = None) -> Dict[str, Any]:
    """创建标准API响应"""
    response = {
        "success": success,
    }
...
def create_paginated_response(data: List[Any], page: int, page_size: int, total: int, message: str = None) -> Dict[str, Any]:
    """创建分页响应"""
