            ...
            return {
                ...
                'endpoint_summary': {
                    key: {
                        ...
                        'success_rate': (self.success_counts[key] / self.total_counts[key]) * 100 if self.total_counts[key] > 0 else 0
                    }
...
class AlertManager:
    ...
    def check_alerts(self) -> List[Dict[str, Any]]:
        ...
        for endpoint, endpoint_stats in stats['endpoints'].items():
            ...
            # 检查端点最大响应时间
            if endpoint_stats['max_response_time_ms'] > self.thresholds['max_response_time_ms']:
                alert = {
...
class MonitoringDashboard:
    ...
    def generate_report(self) -> str:
        ...
        report.append("=" * 60)
        report.append(f"生成时间: {datetime.fromtimestamp(data['timestamp']).strftime('%Y-%m-%d %H:%M:%S')}")
        report.append("")
        ...
        if stats['endpoints']:
            ...
            for endpoint, endpoint_stats in stats['endpoints'].items():
                ...
                report.append(f"     - 平均响应时间: {endpoint_stats['avg_response_time_ms']}ms")
