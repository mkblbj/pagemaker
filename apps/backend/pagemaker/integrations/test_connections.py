...
class RakutenAPITester:
        ...
        try:
            ...
            if connection_result['success']:
                ...
                folders_result = self._safe_api_call(
                    lambda: self.cabinet_client.get_folders(limit=5),
                    "获取文件夹列表"
                )
                ...
                health_result = self._safe_api_call(
                    self.cabinet_client.health_check,
                    "健康检查"
                )
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print(f"测试时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(results['timestamp']))}")
