...
class RCabinetClient:
    ...
    def __init__(
        ...
        self.test_mode = test_mode or os.getenv('RAKUTEN_API_TEST_MODE', TEST_MODE["MOCK"])
        ...
        if self.test_mode == TEST_MODE["MOCK"]:
            self.service_secret = self.service_secret or os.getenv('RAKUTEN_TEST_SERVICE_SECRET')
        ...
        if self.test_mode == TEST_MODE["REAL"]:
            valid, error_msg = validate_credentials(self.service_secret, self.license_key)
            if not valid:
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        if self.test_mode == TEST_MODE["REAL"]:
            headers['Authorization'] = create_auth_header(self.service_secret, self.license_key)
        else:
            # 测试模式动态生成认证头
            headers['Authorization'] = create_auth_header(self.service_secret, self.license_key)
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code != HTTP_STATUS_CODES["OK"]:
                error = map_http_status_to_exception(response.status_code, response.text)
                raise error
            ...
            if result.get('data', {}).get('result_code') is not None:
                ...
                api_error = map_result_code_to_exception(result_code, result.get('interface_id'))
                if api_error:
                    error = api_error
                    raise error
    ...
    def get_folders(
        ...
        return self._make_request('GET', CABINET_ENDPOINTS["FOLDERS_GET"], params=params)
    ...
    def get_folder_files(
        ...
        return self._make_request('GET', CABINET_ENDPOINTS["FOLDER_FILES_GET"], params=params)
    ...
    def search_files(
        ...
        return self._make_request('GET', CABINET_ENDPOINTS["FILES_SEARCH"], params=params)
