...
### 前端多列图文模块组件开发
- [ ] 创建MultiColumnModule组件 (AC: 1, 7) [Source: architecture/frontend-architecture.md#components]
  - [ ] 实现基础多列图文渲染组件 `apps/frontend/src/components/modules/MultiColumnModule.tsx`
  - [ ] 支持四种预设布局的切换功能
  - [ ] 集成到模块列表中，支持拖拽添加到画布
  - [ ] 实现实时预览更新机制

### 布局选择和配置功能
- [ ] 实现布局选择器 (AC: 2, 6) [Source: architecture/frontend-architecture.md#state-management]
  - [ ] 创建四种预设布局模板（图左文右、文左图右、图上文下、文上图下）
  - [ ] 实现布局切换功能和属性面板集成
  - [ ] 设置固定的两列布局列宽比例预设值
  - [ ] 确保布局变更触发预览更新

### 图片区域集成
- [ ] 集成图片配置功能 (AC: 3) [Source: docs/stories/1.6.story.md]
  - [ ] 复用ImageModule的图片上传和选择功能
  - [ ] 支持从R-Cabinet上传/选择图片
  - [ ] 实现alt文本、对齐方式、链接和尺寸配置
  - [ ] 确保图片配置与Story 1.6功能一致

### 文本区域集成
- [ ] 集成文本配置功能 (AC: 4) [Source: docs/stories/1.5.story.md]
  - [ ] 复用TextModule的富文本编辑功能
  - [ ] 支持多段文本内容和格式化（加粗、下划线、超链接）
  - [ ] 实现文本对齐、字体、颜色和背景色配置
  - [ ] 确保文本配置与Story 1.5功能一致

### 响应式HTML导出
- [ ] 实现响应式HTML导出 (AC: 5) [Source: architecture/core-workflows.md#saving-page-content]
  - [ ] 生成支持响应式堆叠的HTML结构
  - [ ] 使用CSS媒体查询实现移动端自动堆叠
  - [ ] 确保导出的HTML在不同设备上正确显示
  - [ ] 集成到现有的HTML导出流程中
...
### 状态管理和数据持久化
- [ ] 扩展状态管理 (AC: 1-7) [Source: architecture/frontend-architecture.md#store-structure]
  - [ ] 扩展usePageStore以支持多列图文模块状态
  - [ ] 实现模块配置数据的JSON序列化
  - [ ] 集成到PUT /api/v1/pages/{id}/ API保存流程
  - [ ] 确保状态变更正确触发预览更新

### 单元测试
- [ ] 编写组件测试 [Source: architecture/testing-strategy.md#frontend-testing]
  - [ ] MultiColumnModule组件单元测试 `apps/frontend/src/components/modules/MultiColumnModule.test.tsx`
  - [ ] 布局切换功能测试
  - [ ] 图片和文本区域配置测试
  - [ ] 响应式布局测试
